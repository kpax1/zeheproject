...
const Form = ({choosedTimes, choosedDate,idEnd}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/booked', {
        ...
        body: JSON.stringify({ choosedDate, choosedTimes, email, name, number }),
      });
  
      if(response.ok){
        console.log('newbookingday is AAAAAAAAAAAAAAAAAAAAAAAAAded to BBOOOOKKKEEDD')
        router.push('/thanks')
      }
  ...
  return (
    ...
    <button onClick={handleSubmit}>
      butttbubutubutbutubutubutbutubutubutubutu
    </button>
      <Title> Please enter your booking details </Title>
        <form onSubmit={handleSubmit}>
        <input type="email" name='email' id='email' placeholder='Email *' value={formData.email} onChange={handleInputChange} />
        <input type="text" name='name' id='name' placeholder='Name *' value={formData.name} onChange={handleInputChange} />
        <input type="number" name='number' id='number' placeholder='Phone number *' value={formData.number} onChange={handleInputChange} />
